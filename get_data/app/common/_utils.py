...
def handle_league_name(league_name, source_type=None):
    ...
    try:
        ...
        if not league:
            ...
            # 注意：分类字段 region_id, country_id, level_id 默认为 NULL
            # 需要通过后续的联赛分类初始化脚本来填充
...
def _auto_fill_league_classification(league):
    """
    自动填充联赛分类字段（region_id, country_id, level_id）
    ...
    Returns:
        bool: 是否成功填充
    """
    from app.database import localdb, LeagueRegion, LeagueCountry, LeagueLevel
    
    try:
        # 1. 构建关键词映射（复用 update_league_classification.py 的逻辑）
        ...
        # 先检查国际赛事
        for keyword, mapping in international_keywords.items():
            if keyword in league_name or keyword in league_abbr:
                matched_country_code = mapping['country_code']
                matched_level_code = mapping['level_code']
                break
        ...
        if not matched_country_code:
            ...
            matched_country_code = best_match
        
        # 3. 匹配等级（如果还没有匹配到）
        if not matched_level_code:
            best_match = None
            best_score = 0
            ...
            matched_level_code = best_match
        
        # 4. 查询并设置分类ID
        updated = False
        
        # 设置国家ID
        if matched_country_code and not league.country_id:
            country = localdb.query(LeagueCountry).filter_by(country_code=matched_country_code).first()
            if country:
                league.country_id = country.id
                # 同时设置地区ID
                if country.region_id and not league.region_id:
                    league.region_id = country.region_id
                updated = True
                db_log.debug(f"✓ 自动匹配国家: {league_name} -> {country.country_name}")
        
        # 设置等级ID
        if matched_level_code and not league.level_id:
            level = localdb.query(LeagueLevel).filter_by(level_code=matched_level_code).first()
            if level:
                league.level_id = level.id
                updated = True
                db_log.debug(f"✓ 自动匹配等级: {league_name} -> {level.level_name}")
        
        # 5. 如果有更新，保存到数据库
        if updated:
            localdb.update(league, close=False)
            db_log.info(f"✅ 自动填充联赛分类: {league_name} (region={league.region_id}, country={league.country_id}, level={league.level_id})")
            return True
        else:
            db_log.debug(f"⊘ 未能匹配联赛分类: {league_name}")
            return False
            
    except Exception as e:
        db_log.error(f"自动填充联赛分类失败 ({league.league_name}): {str(e)}")
        import traceback
        db_log.error(traceback.format_exc())
        return False
...
def get_or_create_league(league_name, league_name_abbr=None):
    ...
    try:
        ...
        if league:
            ...
            # 【新增】自动填充联赛分类字段（region_id, country_id, level_id）
            if not league.region_id or not league.country_id or not league.level_id:
                _auto_fill_league_classification(league)
